...
export default function BestTime(TimeElapsed){
    ...
    if(TimeElapsed < UserData.BestTime || UserData.BestTime === null){
        ...
        fetch(`http://localhost:8000/Server/BestTime/${UserData.Name}`, {
            method: 'PUT',
            headers: {
        ...
        fetch(`http://localhost:8000/Server/UserProfile/${UserData.Name}`)
        .then(response => response.json())
        .then(Data => {
            CurrentUserNameSingleton.setUserName(Data);
            console.log("NEW BEST TIME    "+CurrentUserNameSingleton.getUserName().BestTime);
        })
        .catch(error => console.error('Error:', error));
    }
}